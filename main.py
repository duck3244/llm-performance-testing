...
# 로컬 모듈 import
try:
    from config import RobustConfigManager, ConfigValidator, HardwareDetector, SafetyChecker, InferenceParams
    from test_runner import SafePerformanceOptimizer, SafeModelManager
...
안전한 사용 예시:
  ...
  python main.py optimize --model llama2-7b --dataset korean_math --samples 20
  python main.py benchmark --model mistral-7b --dataset korean_qa --samples 50
...
def run_init_command(args):
    ...
    try:
        ...
        print("3. 안전한 최적화 실행: python main.py optimize --model [모델명] --dataset [데이터셋명] --samples 20")
        print("4. 결과 확인: python main.py visualize --type dashboard")
...
def show_welcome_screen_enhanced():
    ...
    print("4. 안전한 최적화: python main.py optimize --model [모델명] --dataset [데이터셋명] --safe-mode")
